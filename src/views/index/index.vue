...
<template>
  ...
  <section class="pt-5">
    <div class="container max-w-screen-xl mx-auto px-4">
      <div class="grid md:grid-cols-1 gap-5">
        <div class="hero object-cover h-[400px] rounded-lg"
             :style="{ backgroundImage: 'url(' + randomProduct.thumbnail + ')' }">
  ...
  <section class="pt-10">
    <div class="container max-w-screen-xl mx-auto px-4">
      <nav class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-10 gap-x-3 gap-y-6">
        <category-icon v-for="category in categoryStore.categories" :key="category" :category="category"/>
      </nav>
    </div>
    <!-- container //end -->
  </section>
  ...
  <div class="flex items-center justify-center py-10">
    <button v-if="checkMoreItem" :class="{ loading: loading }" class="btn btn-primary btn-sm" @click="loadMore">Load
      more
    </button>
