...
onMounted(async () => {
  ...
  await categoryStore.categoryList(100);
  loading.value = false;
});
</script>
      ...
      <div class="flex flex-col md:flex-row -mx-4">
        <aside class="md:w-1/3 lg:w-1/4 px-4">
          ...
          <button @click="showFilters = !showFilters"
                  class="md:hidden mb-5 w-full text-center px-4 py-2 inline-block text-lg text-gray-700 bg-white shadow-sm border border-gray-200 rounded-md hover:bg-gray-100 hover:text-primary"
                  href="#">
            Filter by
          </button>
          ...
          <transition name="slide">
            <div v-show="showFilters" class="md:hidden px-6 py-4 bg-base-300 rounded shadow-sm mb-5">
              ...
              <ul class="text-gray-500 space-y-1 h-48 overflow-auto">
                ...
                <li v-for="category in categoryStore.categories" :key="category.value">
                  <a :href="$router.resolve({ name: 'category', params: { slug: category.value } }).href"
                     class="hover:text-primary"
                     :class="{ 'font-semibold text-primary': category.value === filters.category }">
                    {{ category.label }}
                  </a>
                </li>
              </ul>
